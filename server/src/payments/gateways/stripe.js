// src/payments/gateways/stripe.js
...
const supportedCountries = require('../../utils/constants').paypalSupportedCountries; // Reuse for Stripe

function validateCountry(countryCode) {
  if (!supportedCountries.includes(countryCode)) {
    throw new ErrorHandler(400, 'Stripe not supported in your country');
  }
}
...
async function sendNotifications(userId, status, amount, currency, transactionId) {
  try {
    ...
    logger.error(`Stripe notification error for user ${userId}: ${err.message}`);
  }
}
...
exports.processPayment = async (options) => {
  ...
  if (!details.cardNumber || !details.cvv || !details.expiryMonth || !details.expiryYear) {
    throw new ErrorHandler(400, 'Card details required for Stripe');
  }
  ...
  try {
    ...
    logger.info(`Stripe payment initiated: ${amount} ${currency} for user ${userId}`);
    ...
    throw new ErrorHandler(500, 'Stripe payment processing failed');
  }
};
...
exports.capturePayment = async (transactionId, userId, paymentId) => {
  try {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      logger.info(`Stripe payment captured: ${transactionId}`);
      return { success: true, transactionId };
    } else if (paymentIntent.status === 'requires_action' || paymentIntent.status === 'requires_confirmation') {
      logger.info(`Stripe payment requires action: ${transactionId}`);
      ...
      logger.error(`Stripe capture failed: ${paymentIntent.status}`);
      return { success: false };
    }
  } catch (err) {
    logger.error(`Stripe capture error: ${err.message}`);
    throw new ErrorHandler(500, 'Stripe capture failed');
  }
};
...
exports.processRefund = async (options) => {
  ...
  try {
    ...
    logger.info(`Stripe refund processed: ${amount} USD for ${originalTransactionId}`);
    ...
    throw new ErrorHandler(500, 'Stripe refund processing failed');
  }
};

exports.processWithdrawal = async (options) => {
  const { amount, currency, details, userId } = options;
  if (!details.accountNumber) throw new ErrorHandler(400, 'Bank account required for Stripe');
  ...
  try {
    ...
    logger.info(`Stripe payout initiated: ${amount} ${currency} to ${details.accountNumber}`);
    await sendNotifications(userId, 'success', amount, currency, payout.id);
    return { success: true, transactionId: payout.id };
  } catch (err) {
    logger.error(`Stripe payout error: ${err.message}`);
    throw new ErrorHandler(500, 'Stripe payout processing failed');
  }
};
...
exports.handleCallback = async (payload) => {
  try {
    ...
    logger.info(`Stripe webhook processed: ${event.type} for ${transactionId}`);
    return { success, txId: transactionId };
  } catch (err) {
    logger.error(`Stripe webhook error: ${err.message}`);
    throw new ErrorHandler(500, 'Stripe webhook processing failed');
  }
};